function jtorch._saveSpatialDivisiveNormalizationNode(node, ofile)

  if (node.kernel:dim() > 2) then
    error("saveSpatialDivisiveNormalizationNode() - ERROR: Only 1D and 2D kernels are supported for now!")
  end
