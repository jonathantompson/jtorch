...
class Clk {
 public:
  ...
  double getTime() {
    ...
    elapsedNano =
        (m_cntCurrent - m_cntStart) * sTimebaseInfo.numer / sTimebaseInfo.denom;
...
class Clk {
 public:
  Clk() { clock_gettime(CLOCK_REALTIME, &m_cntStart); }

  double getTime() {
    lock_.lock();
    clock_gettime(CLOCK_REALTIME, &m_cntCurrent);
    elapsedNano =
        (double)(m_cntCurrent.tv_nsec - m_cntStart.tv_nsec) / 1000000000.0;
