...
const uint32_t kTestHMMStartSize = 101;  // A "bigish" prime
const uint32_t kTestHMMNumValues = 52;  // Enough to force at least one re-hash

using jcl::data_str::HashMapManaged;
using jcl::data_str::HashUInt;
using jcl::data_str::HashString;

// TEST 1: Create a hash table, insert items from 1:N
TEST(HashMapManaged, CreationAndInsertion) {
  HashMapManaged<uint32_t, uint32_t> ht(kTestHMMStartSize, &HashUInt);
...
TEST(HashMapManaged, StringCreationAndInsertion) {
  HashMapManaged<std::string, uint32_t> ht(kTestHMMStartSize, &HashString);
  std::stringstream ss;
